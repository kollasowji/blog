...
mydb=mysql.connector.connect(host="localhost",user="root",password="3010",database="flaskblog")
with mysql.connector.connect(host="localhost",user="root",password="3010",database="flaskblog"):
...
def update_post(id):
    if request.method=="POST":
        ...
        mydb.commit()
        cursor.close()                 
        return render_template(url_for('viewpost'))
    else:
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select * from posts where id=%s',(id,))
        post=cursor.fetchone()
